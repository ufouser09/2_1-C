...
#include <math.h>
#include <time.h>
#include <windows.h>//OYUNU TAM EKRANDA ACMAMIZ ICIN GEREKEN FONKSÝYON
#include <conio.h>//GETCH'Ý KULLANMAMIZI SAÐLAYAN FONKSÝYON
...
int main(){
	...
	ShowWindow(GetConsoleWindow(), SW_SHOWMAXIMIZED);//OYUNUN TAM EKRANDA ACILMASI ÝÇÝN GEREKEN FONKSÝYON
	system("COLOR F0");//EKRAN VE YAZI RENGÝ ÝÇÝN FONKSÝYON
...
//*****************************************************************************
//KULLANICI ADI TXT DOSYASINDA BULUNUYORSA KULLANICIDAN BU BÝLGÝLERÝ TEKRAR AL
...
int hash(char *str, int length) {
    int hash = 5381;//HASH DEGÝSKENÝN BASLANGIC NOKTASI
    printf("nick: %s",str);
    int c;
...
void kullaniciEkle(USER** allUsers, USER* user, int hashDegeri) {
    ...
    if (allUsers[hashDegeri] == NULL) {
        ...
        user->nextUser = allUsers[hashDegeri];  
...
int haritalar(){
    ...
    if(matris == NULL) {//EGER NULL DONERSE BELLEKTE YETERÝNCE YER YOK DEMEKTÝR. 
        fprintf(stderr, "Matris oluþturulamadý.\n");
        return 1;
    }

    // OLUSTURULAN MATRISI DOSYAYA KAYDET
    char* dosyaAdi = "matris.txt"; // KAYDEDÝLECEK DOSYANIN ADI
...
char* sayiSembolDonustur(int sayi){
    // STATÝK BÝR DÝZÝ DÖNDURMEK ÝÇÝN TANIMLIYORUZ KÝ DÝZÝ FONKSÝYON BÝTÝNCE KAYBOLMASIN
    ...
    switch(sayi){
        ...
        case 6:
            ...
            break;
        default:
            strcpy(sembol, "??\0"); // TANIMSIZ BÝR SAYI GÝRÝLDÝGÝNDE
    }
    return sembol;
}
void matrisiDosyayaKaydet(char* dosyaAdi, char*** matris, int satir, int sutun) {
    ...
    int i, j;
    if (file == NULL) {//EGER NULL DONERSE DOSYA ACILAMAMISTIR DEMEKTÝR
        perror("Dosya açýlamadý");
        return;
    }

    fprintf(file, "%d %d\n", satir, sutun); // SATIR VE SUTUN SAYISINI DOSYAYA YAZ

    //MATRÝSÝ DOSYAYA YAZ
    for (i = 0; i < satir; i++) {
        for (j = 0; j < sutun; j++) {
            fprintf(file, "%d ", sembolSayiDonustur(matris[i][j])); // HER BÝR MATRÝS ELEMANINI DOSYAYA YAZ
        }
        fprintf(file, "\n"); // HER SATIRDAN SONRA NEWLÝNE KARAKTERÝ EKLE
    }
...
char*** kullaniciMatrisiOlustur(int *satir, int *sutun) {
    ...
    // KULLANICIYA SEMBOLLERÝN ANLAMLARINI GOSTER
    sembollerinAnlamlariniGoster();

    // MATRÝSÝ ALLOCATE EDÝYORUZ
    ...
    // KULLANICIDAN MATRÝSÝ ELEMANLARINI AL
    for(i = 0; i < *satir; i++) {
        for(j = 0; j < *sutun; j++) {
            ...
            strcpy(matris[i][j], sayiSembolDonustur(sayi)); // SEMBOL DONUSUMUNU UYGULA
        }
    }

    return matris; // OLUSTURULAN MATRÝSÝ DONDUR
}
char*** matrisOkuVeDonustur(char* dosyaAdi, int *satir, int *sutun) {
	int i,j;
